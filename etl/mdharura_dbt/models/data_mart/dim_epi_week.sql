...
with
    ...
    dates as (select date as date_day from date_range),
    epi_week_cte as (
        select distinct
    ...
    summary as (
        select
            ...
            case
                when
                    date_part('month', end_of_week) = 1
                    and date_part('week', end_of_week) in (52, 53)
                then date_part('year', end_of_week) - 1
    ...
    final_data as (
        select
            {{ dbt_utils.generate_surrogate_key(["summary.start_of_week"]) }}
            as epi_week_key,
            start_of_week,
            end_of_week,
            week_number,
